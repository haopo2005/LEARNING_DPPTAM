...
SemiDenseTracking::SemiDenseTracking()
{
   ...
   /*
     * local_maps_number are the potential past keyframes to be used in tracking
     *local_maps_close_number are the number of past keyframes used in tracking. 
     *They are selected from 'local_maps_number'.
     */
...
{
    /*
      'cont_frames' is the counter of the last frame, either if it has already been tracked or not.
      'last_cont_frames' is the counter of the last frame tracked
      The purpose of these variables is to realize when a new frame is ready for tracking
	*/
    if  (*semidense_tracker->cont_frames > semidense_tracker->last_cont_frames)
    {
            ...
            if (semidense_tracker->image_n > semidense_tracker->last_frame_tracked)
            {
			 ...
			 /*
			 *initialization of tracking (the jacobians have to be recalculated) is performed 
			 *when do_initialization_tracking > 0.5.
			 *do_initialization_tracking is set to 1 in the semidense mapping thread,
			 *when the depth of the new keyframe is calculated and then it is ready to be tracked.
			 */
             if (semidense_mapper->do_initialization_tracking > 0.5 )
             {
            ...
            if (semidense_mapper->do_initialization > 0.5)
            {
			 ...
			 /*
			 *points3D_cam_to_print are just the 3D points that are going to be printed 
			   in the current frame for visualization.
			 */
                   ...
                   double &fx,double &fy, double &cx, double &cy, int distortion, int reduction)
{
    if (distortion > 0.5)
    {
         ...
         double alpha = 0;
		 //得到自由比例参数的新摄像机矩阵
		 //alpha – Free scaling parameter between 0 and 1, 0 means all the pixels in the undistorted image are valid
                    ...
                    double cx,double cy,  vector<double> &centerx,vector<double> &centery, int reduction,cv::Mat image_gray,float limit_grad)
{
   ...
   image_rgb.copyTo(image_keyframe);  //第一帧做关键帧
   ...
   double depth_aux = depth;  //depth = 0.020*50，最初的
   ...
   for (int i = 0; i < image_red.cols-0; i = i+1)
   {
           ...
           //此处逐像素point前3列为相机坐标系XYZ, depth是假的
   ...
   for (int j=0; j<pyramid_levels ; j++)
   {
		 ...
		 //根据reduction的值，确定高梯度点的位置及颜色，保存到points_map中
...
//将当前高梯度点世界坐标分别转换到前一帧与当前帧的成像平面上，
//再将两个时刻的投影点与高梯度世界坐标平面求差，接受误差小的
//作为当前相机的POSE (哪个角度看高梯度点清楚，就用哪个POSE延续)
                  ...
                  vector<cv::Mat> &image_keyframe_pyramid,int pyramid_levels, bool &good_seed)
{
    ...
    int i = 0;
    get_color(points_map[i],color[i]);//统一高梯度的颜色, color是空间中的平面
    cv::Mat error_vector(points_map[i].rows,1,CV_64FC1);
    cv::Mat error_vector_sqrt(points_map[i].rows,1,CV_64FC1);
    ...
    cv::Mat pointsClouds3D =  points_map[i].t();
    pointsClouds3D = pointsClouds3D .rowRange(0,3); //分别取三层金字塔的高梯度点XYZRGB
    //高梯度点XYZRGB转换为图像平面坐标
    ...
    cv::Mat t1 =  R*t_rel+t; //R,T由函数参数导入，为当前帧的R,T；t_rel,R_rel为前一帧的R,T
    cv::Mat R1 =  R*R_rel;
                     ...
                     ros::Publisher *odom_pub,ros::Publisher *pub_poses,ros::Publisher *vis_pub,cv::Mat &points3D_cam_to_print)
{
    ...
    cv::Mat t_aux2 =  R*t_rel+t; //R,T源自函数参数semidense_tracker->R,semidense_tracker->t
    cv::Mat R_aux2 =  R*R_rel;//此处为累积之前关键帧POSE关系

    /*
        *The seed for the transformation Tn comes from a constant
        *velocity motion model –although this step is ignored if the
        *photometric reprojection error is higher after applying it.
	*/
    /// MOTION MODEL，可有可无的步骤，增加/减少gauss-newton 收敛速度
    if (num_keyframes >  0 )
    {
           t_aux2 =  R*t_rel+t;
            R_aux2 =  R*R_rel;
    ...
    R1.convertTo(R1,CV_64FC1);//R1,T1是当前keyframe的
...
void resize_points(cv::Mat  &points2,cv::Mat  &points, double reduction,cv::Mat &image_p, int kinect_initialization,float limit_grad)
{
    ...
    cv::Mat G = cv::abs(GX)  + cv::abs(GY); //两个方向梯度相加
    ...
    double variance = sorted_depths.at<double>(round(sorted_depths.rows/2),0);//sorted_depths求中值
...
//分别将投影后的成像平面点做插值，并与当前该点颜色求差[此处颜色是归一化后的]
              ...
              vector<cv::Mat> &image_keyframe_pyramid, float  &points_projected_in_image)
{
    ...
    for (int i = 0; i < pyramid_levels;i++)
    {
        ...
        /*
            *check_repatead_points is to be sure that the system does not track the same 3D point twice. 
            *When I fuse two keyframes I make sure that the resulting 3D points are all different.
		*/
