...
/*
*All 3D points are not saved. They are just printed as .ply files 
(you can find them in the folder dpptam/src/map_and_poses).
But you can save them if you need them. 
The semidense map tracked can be found in the variable 'semidense_tracker->points_map_inImage'
3D planar superpixels can be found in the following variable:
dense_mapper->get_superpixels3Dtracked()
*/
...
void semidense_mapping(DenseMapping *dense_mapper,SemiDenseMapping *semidense_mapper,SemiDenseTracking *semidense_tracker,MapShared  *Map,Imagenes *pimages,Imagenes  *pimages_previous_keyframe,ros::Publisher *pub_cloud)
{
				...
				//Minimum parallax required for mapping
				//Minimum parallax to insert a keyframe. Default: 0.075. Typical values [0.03-0.15].
                ...
                int num_keyframes = semidense_mapper->num_keyframes;//初始化有0关键帧
               ...
               semidense_mapper->previous_images = semidense_mapper->num_cameras_mapping_th/2+1+1;
                ...
                if (semidense_mapper->do_var_mapping > 0.5)
                {
                    ...
                    join_last_images(pimages,pimages_previous_keyframe,dense_mapper,semidense_mapper); //插入关键帧
                    ///////////////////////////////////////////////////////////////
...
void copy_first_and_last_images(Imagenes &images, Imagenes &images_map)
{
    ...
    for (int l = 0 ; l < images_size; l = l+1)
    {
        ...
        {
            images_map.computeImage(); //每computeImage一次，就增加一个Imagen对象，images_map_size数量加1
